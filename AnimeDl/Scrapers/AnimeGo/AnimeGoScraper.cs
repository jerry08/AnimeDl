...
public class AnimeGoScraper : BaseScraper
{
    ...
    public override async Task<List<VideoServer>> GetVideoServersAsync(Episode episode)
    {
        ...
        var servers = doc.DocumentNode.SelectNodes(".//div[@class='anime_muti_link']/ul/li").ToList();
    ...
    public override async Task<List<Genre>> GetGenresAsync()
    {
        ...
        var genresNode = document.DocumentNode.Descendants().Where(node => node.GetClasses().Contains("genre")).FirstOrDefault();
